...
FWGeometry::getHGCalRecHits(
   ...
   else
   {
      ...
      float centerX = (info.points[0] + info.points[0 + offset]) / 2;
      float centerY = (info.points[1] + info.points[1 + offset]) / 2;
      float radius = fabs(info.points[1] - info.points[1 + offset]) / 2;
      silicon->AddHex(TEveVector(centerX, centerY, info.points[2]),
                           radius, 0.0, info.shape[3]);
