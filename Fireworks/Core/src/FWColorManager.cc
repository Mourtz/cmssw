...
void FWColorManager::setDefaultGeomColors() {
  ...
  m_geomColor[kFwHFNoseColorIndex] = 1005;
...
int FWColorManager::brightness() { return TMath::FloorNint(-m_gammaOff * 10); }
...
void FWColorManager::switchBackground() { setBackgroundColorIndex(isColorSetDark() ? kWhiteIndex : kBlackIndex); }
...
void FWColorManager::setBackgroundAndBrightness(BackgroundColorIndex iIndex, int b) {
  m_gammaOff = -b * 0.1f;
  setBackgroundColorIndex(iIndex);
}

Bool_t FWColorManager::setColorSetViewer(TGLViewer* v, Color_t iColor) {
  if ((iColor == kBlackIndex && !v->IsColorSetDark()) || (iColor == kWhiteIndex && v->IsColorSetDark())) {
    v->SwitchColorSet();
    return kTRUE;
  }
  return kFALSE;
...
void FWColorManager::setGeomColor(FWGeomColorIndex idx, Color_t iColor) {
  // printf("set geom color %d \n", iColor);
  m_geomColor[idx] = iColor;
  geomColorsHaveChanged_();
  gEve->Redraw3D();
}
void FWColorManager::setGeomTransparency(Color_t iTransp, bool projectedType) {
  if (projectedType)
...
Color_t FWColorManager::geomColor(FWGeomColorIndex iIndex) const { return m_geomColor[iIndex]; }

static std::shared_ptr<std::map<Color_t, Color_t> > m_oldColorToIndexMap;

Color_t FWColorManager::oldColorToIndex(Color_t iColor, int version) const {
  enum {
    kFWRed = 1008,
    kFWBlue = 1005,
    kFWCyan = 1007,
